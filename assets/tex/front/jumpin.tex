...
\par
After the conditional we have a library call \texttt{std.out(x)}. This call prints the value of x to the screen. \texttt{std.out} in Jac is equivalent to the the \texttt{print} in Python and analogous to the \texttt{printf}, \texttt{cout}, and \texttt{console.log} you'd find in C, C++, and Javascript respectively. A suite of core standard library operations for the language has the preamble of \texttt{std}.
...
\jacdotnw{tldr_staticgraph}{1}{Graph in memory for JC~\ref{jac:tldr_staticgraph.jac}}
...
\subsection{Use Imports}
\subsection{Test Driven Development}
\subsection{File I/O}

\subsection{AI with JSKit}
[Adding some AI]
\jaccode{tldr_ai.jac}{First Example}
Output:
